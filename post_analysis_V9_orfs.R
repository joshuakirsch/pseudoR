...
# Build main listing of sites ---------------------------------------------

for (i in seq (nrow(sampleList))) {
  temp_IS = read_tsv(paste ("mapping_files/",sampleList$Sample[i], ".unmapped.orf_mapping.filtered.sam", sep=""), col_names = sam_file_cols,
                     show_col_types = FALSE) %>%
    mutate (sample = sampleList$Sample[i]) %>%
    select (c(sam_file_cols, sample))
    IS_annot = read_csv(paste ("mapping_files/",sampleList$Sample[i], "_pre_mapping_output.csv", sep=""),
                        show_col_types = FALSE) %>%
